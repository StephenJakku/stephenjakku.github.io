...
function Intro({ refer }) {
  return (
    <div
      className="min-h-screen snap-start flex flex-col justify-center bg-hero-gradient-light dark:bg-hero-gradient px-8"
      ref={refer}
    >
      <motion.div
      ...
      >
        ...
        <motion.h1 variants={item} className="text-6xl md:text-7xl font-bold tracking-tight bg-gradient-to-r from-slate-900 via-slate-800 to-accent dark:from-white dark:via-white dark:to-accent bg-clip-text text-transparent">
          {personal.name}
        </motion.h1>
        ...
        <motion.div variants={item} className="flex gap-4 pt-2">
          ...
          <button
            ...
            className="border border-slate-400 dark:border-white/30 text-slate-700 dark:text-white px-6 py-2.5 rounded hover:border-slate-700 dark:hover:border-white transition-colors text-sm"
          >
            View Resume
          </button>
        </motion.div>
